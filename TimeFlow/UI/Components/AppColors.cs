using System.Drawing;

namespace TimeFlow.UI.Components
{
    /// <summary>
    /// Centralized color palette for consistent UI theming across the application
    /// </summary>
    public static class AppColors
    {
        // Gray Scale
        public static Color Gray50 = ColorTranslator.FromHtml("#F9FAFB");
        public static Color Gray100 = ColorTranslator.FromHtml("#F3F4F6");
        public static Color Gray200 = ColorTranslator.FromHtml("#E5E7EB");
        public static Color Gray300 = ColorTranslator.FromHtml("#D1D5DB");
        public static Color Gray400 = ColorTranslator.FromHtml("#9CA3AF");
        public static Color Gray500 = ColorTranslator.FromHtml("#6B7280");
        public static Color Gray600 = ColorTranslator.FromHtml("#4B5563");
        public static Color Gray700 = ColorTranslator.FromHtml("#374151");
        public static Color Gray800 = ColorTranslator.FromHtml("#1F2937");
        public static Color Gray900 = ColorTranslator.FromHtml("#111827");

        // Blue
        public static Color Blue50 = ColorTranslator.FromHtml("#EFF6FF");
        public static readonly Color Blue100 = Color.FromArgb(225, 240, 255);
        public static Color Blue500 = ColorTranslator.FromHtml("#3B82F6");
        public static Color Blue600 = ColorTranslator.FromHtml("#2563EB");

        // Green
        public static Color Green500 = ColorTranslator.FromHtml("#10B981");
        public static Color Green600 = ColorTranslator.FromHtml("#059669");

        // Red
        public static Color Red50 = ColorTranslator.FromHtml("#FEF2F2");
        public static Color Red200 = ColorTranslator.FromHtml("#FECACA");
        public static Color Red500 = ColorTranslator.FromHtml("#EF4444");
        public static Color Red600 = ColorTranslator.FromHtml("#DC2626");
        public static Color Red700 = ColorTranslator.FromHtml("#B91C1C");

        // Yellow & Orange
        public static Color Yellow500 = ColorTranslator.FromHtml("#F59E0B");
        public static Color Orange500 = ColorTranslator.FromHtml("#F97316");
        public static Color Orange600 = ColorTranslator.FromHtml("#EA580C");

        // Purple
        public static Color Purple500 = ColorTranslator.FromHtml("#8B5CF6");
        public static Color Purple600 = ColorTranslator.FromHtml("#7C3AED");

        // Basic Colors
        public static Color White = Color.White;
        public static Color Black = Color.Black;

        // Special Colors
        public static Color MenuBorderColor = Color.Black;
    }
}
