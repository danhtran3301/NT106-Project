...
namespace TimeFlow.Data.Repositories
{
    ...
    public class MessageRepository
    {
        ...
        public void AddGroupMessage(string sender, int groupId, string content)
        {
            ...
            catch (Exception ex) { Console.WriteLine("Error Group Msg: " + ex.Message); }
        }
        ...
        public List<MessageData> GetGroupHistory(int groupId)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT SenderUsername, Content, Timestamp FROM Messages WHERE GroupId = @g ORDER BY Timestamp ASC";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@g", groupId);
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                list.Add(new MessageData { Sender = reader.GetString(0), Content = reader.GetString(1), Time = reader.GetDateTime(2) });
                            }
                        }
                    }
                }
                ...
                using (SqlConnection conn = new SqlConnection(_connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                list.Add(new MessageData { Sender = reader.GetString(0), Receiver = reader.GetString(1), Content = reader.GetString(2), Time = reader.GetDateTime(3) });
                            }
                        }
                    }
                }
